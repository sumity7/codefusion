...
function AdminGuard(){useDocumentTitle("Admin");const{user,loading}=useAuth();const location=useLocation();if(loading)return <main className="simple-page container"><p>Checking admin access…</p></main>;if(!user||user.role!=="admin")return <Navigate to={`/admin/login?next=${encodeURIComponent(location.pathname)}`} replace/>;return <Admin/>}
